...
new Server({
  routes() {
    ...
    this.post('/comments', function (schema, req) {
      const { id, text, author } = JSON.parse(req.requestBody)

      // const post = schema.posts.find(id)
      const user = schema.users.find(author)
    ...
    this.post('/commentreply', function (schema, req) {
      const { commentId, postId, text, author } = JSON.parse(req.requestBody)
